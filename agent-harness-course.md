...
| 模块 | 主题 | 关键产出 |
|---|---|---|
| 模块 0 | 环境与第一次 API 调用 | 跑通最小 LLM 调用 |
| 模块 1 | 手写 Agent Loop | 100 行的最小 Agent |
| 模块 2 | 工具设计 | 一套健壮的工具接口 |
| 模块 3 | Context 管理 | 不会爆 context 的 Agent |
| 模块 4 | 失败模式与容错（Harness 入门） | 能从错误中恢复的 Agent |
| 模块 5 | 权限与沙箱 | 危险操作有确认门的 Agent |
| 模块 6 | 可观测性：Trace 与 Events | 可被完整 debug 的 Agent |
| 模块 7 | 多 Agent 编排 | 主-子 agent 编排器 |
| 模块 8 | 阅读真实 Harness | 一份源码精读笔记 |
| 结课项目 | 综合实战 | 一个完整的领域 Agent |

建议节奏：模块 0-3 约两周，模块 4-7 约三到四周（模块 4 最关键，别赶），模块 8 + 结课项目按兴趣延展。
...
- **幻觉调用**：模型调用不存在的工具或编造参数。Harness 层拦截并纠正。
...
- **人工确认门（human-in-the-loop）**：危险/不可逆操作必须暂停等人确认。如何在 loop 里实现"挂起-等待-恢复"。
...
- 复用模块 6 的 trace，让编排器的每个子 agent 都可观测。
...
**学习目标**：带着前 7 章踩过的坑，去读成熟 Harness 的源码，把零散经验系统化。

**核心内容**
- 推荐阅读对象（任选）：开源 Agent 运行时、Coding Agent 项目、或内部 Harness。
- 带着具体问题读，而不是从头读到尾：
  - 它怎么管理 context？（对应模块 3）
  - 它怎么处理工具错误和死循环？（模块 4）
  - 它的权限/沙箱模型长什么样？（模块 5）
  - 它的 trace/events 怎么设计？（模块 6）
  - 它怎么做子 agent / 编排？（模块 7）

**动手练习**
- 选一个真实 Harness，针对上面 5 个问题各写一段精读笔记。
...
把所有模块综合成一个真正解决问题的 Agent。建议选一个你自己领域的真实任务：
...
**结课项目检查清单**（一份合格 Harness 的最低要求）：
- [ ] 手写的 agent loop（不依赖重型框架）
- [ ] 一套有校验、错误能优雅返回的工具
- [ ] context 管理策略，长任务不崩
- [ ] 失败恢复：能处理工具错误、死循环、限流
- [ ] 权限模型：危险操作有人工确认门，默认最小权限
- [ ] 完整 trace：每次运行可回放、可 debug
- [ ] （进阶）多 agent 编排
...
本课程以静态网站形式呈现，GitHub Pages 直接托管，不需要构建步骤。
...
| 部署 | GitHub Pages（`main` 分支根目录） | 推送即上线 |

**备选**：内容膨胀后可迁移到 VitePress / Docusaurus（内容是 Markdown，迁移成本低）。
...
**排版**：h1 2rem / h2 1.6rem / h3 1.25rem / 正文 1rem 行高 1.75。模块之间 4rem 间距。
...
| 当前模块高亮 | `IntersectionObserver` 动态加 `.active` | 必须 |
...
| Markdown 元素 | 网页呈现 |
|---|---|
| `# 模块 N：标题` | `<section id="module-N"><h2>` |
| `**学习目标**` | 目标卡片组件 |
| `**核心内容**` + 列表 | 正文 + `<ul>` |
| `**动手练习**` + 列表 | 练习卡片（带序号） |
| `**产出物**` | 产出物高亮块 |
...
```text
├── assets/images/            # 架构图、流程图
└── examples/module-N/        # 每个模块的骨架代码和参考实现
```

每个模块目录建议包含：`README.md`（讲义）、`starter/`（骨架代码）、`solution/`（参考实现）、`exercises.md`（练习）。

## 部署步骤

1. 创建 GitHub 仓库 `agent-harness-course`
2. 推送 `index.html` + `README.md` + `agent-harness-course.md` 到 `main`
3. Settings → Pages → Source 选 `main` / `/ (root)`
4. 访问 `https://<username>.github.io/agent-harness-course/`
